...
export const Route = createFileRoute("/admin/khqr-debug")({
  head: () => ({
    meta: [
      { title: "KHQR Debug — Dyna Store Admin" },
      ...
      <div className="max-w-4xl mx-auto space-y-6">
        ...
        <header>
          ...
          <p className="text-sm text-muted-foreground">
            Paste a KHQR payload (the text content of the QR, starts with <code>00020101...</code>) to inspect TLV tags,
            verify CRC16, and surface common Q0626 rejection causes.
          </p>
        </header>
        ...
        {result && (
          <div className="space-y-4">
            <StatusBanner ok={result.ok} crcValid={result.crc.valid} errorCount={result.errors.length} />

            <Section title="CRC16">
              <div className="font-mono text-sm space-y-1">
                <div>declared: <span className={result.crc.valid ? "text-emerald-400" : "text-red-400"}>{result.crc.declared ?? "—"}</span></div>
                <div>computed: <span className="text-emerald-400">{result.crc.computed || "—"}</span></div>
...
function StatusBanner({ ok, crcValid, errorCount }: { ok: boolean; crcValid: boolean; errorCount: number }) {
  if (ok) return (
  ...
  return (
    <div className="font-mono text-xs" style={{ marginLeft: depth * 16 }}>
      ...
      {tag.warnings.map((w, i) => (
        <div key={i} className="ml-1 mt-0.5 text-[11px] font-sans text-amber-400 flex items-start gap-1">
          <AlertTriangle className="h-3 w-3 mt-0.5 shrink-0" /> {w}
        </div>
      ))}
      {tag.children && tag.children.length > 0 && (
        <div className="mt-2 space-y-2 border-l border-border/50 pl-3">
          {tag.children.map((c, i) => <TagRow key={i} tag={c} depth={depth + 1} />)}
        </div>
      )}
    </div>
  );
