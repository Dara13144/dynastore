...
DECLARE
  ...
  _bal integer;
BEGIN
  UPDATE public.topup_requests AS tr
     SET status = 'approved',
         reviewed_at = now(),
         bakong_verified_at = now(),
         bakong_response = _bakong_response
   WHERE tr.id = _request_id AND tr.status = 'pending'
   RETURNING tr.user_id, tr.coins, tr.amount_usd INTO _user, _coins, _amount;
  ...
  SELECT w.balance INTO _old FROM public.wallets w WHERE w.user_id = _user FOR UPDATE;
